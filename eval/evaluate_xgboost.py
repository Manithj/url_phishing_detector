...
===============================
Evaluates the XGBoost (Machine Learning) phishing detection model individually
using a shuffled subset of the dataset.

Usage:
    python -m eval.evaluate_xgboost
    or
    cd final_build && uv run python eval/evaluate_xgboost.py
"""
...
from phishing_detector.classifiers import MLClassifier
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            Path(__file__).parent / "evaluation_xgboost.log",
            encoding="utf-8"
        ),
    ],
)
logger = logging.getLogger("XGBoostEvaluation")


class XGBoostEvaluator:
    """
    Evaluates the XGBoost (ML) model for phishing detection.
    """
    ...
    def __init__(
        ...
        logger.info(f"Initializing XGBoost evaluator with {sample_size} samples")
    ...
    def evaluate(self, save_results: bool = True) -> Dict:
        ...
        logger.info("Initializing XGBoost (ML) Classifier...")
        ...
        try:
            classifier = MLClassifier(ml_model_path, ml_vectorizer_path)
            logger.info("XGBoost Classifier initialized successfully")
        except Exception as e:
            logger.error(f"Failed to initialize XGBoost classifier: {e}")
            return {}
        
        self.start_time = datetime.now()
        logger.info(f"Starting XGBoost evaluation at {self.start_time}")
        ...
        logger.info(f"XGBoost evaluation completed in {duration:.1f} seconds")
    ...
    def calculate_metrics(self) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info(f"Model: XGBoost (XGBoost-ML)")
    ...
    def save_results(self, metrics: Dict):
        ...
        results_path = output_dir / f"xgboost_evaluation_results_{timestamp}.csv"
        results_df.to_csv(results_path, index=False)
        logger.info(f"Detailed results saved to: {results_path}")
        
        # Save metrics to JSON
        metrics_path = output_dir / f"xgboost_evaluation_metrics_{timestamp}.json"
        with open(metrics_path, 'w') as f:
            json.dump(metrics, f, indent=2)
        logger.info(f"Metrics saved to: {metrics_path}")
        
        # Also save a summary report
        report_path = output_dir / f"xgboost_evaluation_report_{timestamp}.md"
        with open(report_path, 'w') as f:
            f.write("# XGBoost Model Evaluation Report\n\n")
            f.write(f"**Model:** XGBoost (XGBoost-ML)\n\n")
...
def main():
    """Main entry point for XGBoost evaluation."""
